...
\hline
        & moodle  & degree & junwang & alphago &IRDM &computer graphics syllabus&bioinformatics \\ \hline
number of relevant links& 2& 87 &30&4&62&75&36\\ \hline
number of irrelevant links& 67&65&71&26&34&44&33\\ \hline
total number of links &69&152&101&30&96&119&69\\ \hline
\end{tabular}%
}
\end{table}

=======
The test set was manually created in order for us to evaluate the performance of our search engine. Two main test sets were created: one for navigational use, and one for informational use. This means that the former test set is specifically created such that \emph{one} specific document is most relevant (precision is key), while the latter informational test set is created to contain as many relevant documents as possible (recall is key). Both test sets include queries and relevant documents related to the queries, so that recall can be calculated. In total, about 100 links for each query were labeled, with the number of relevant documents depending on the query.
>>>>>>> origin/master
...
\begin{figure}
\centering
\includegraphics[height=1.5in, width=2in]{figure5}
...
Another aspect to take into consideration when evaluating performance of an algorithm is the time taken to rank the documents. Table \ref{tab:time} shows the average time of 10 queries of different lengths, both with and without PageRank. These are delays observed by the end user, and could be shortened if running the search engine on the Amazon EC2 server, where it has faster access to the database.
